...
async def test_start_game_succeeds_when_passed_a_user_id(aiohttp_client, application, user_id):
...
async def test_start_game_responds_with_user_and_game_id(aiohttp_client, application, user_id):
    client = await aiohttp_client(application)
    response: client.ClientResponse = await client.post('/game', json={
        'user_id': user_id
    })
...
async def test_start_game_user_id_matches_received(aiohttp_client, application, user_id):
    client = await aiohttp_client(application)
    response: client.ClientResponse = await client.post('/game', json={
        'user_id': user_id
    })
...
async def test_start_game_game_id_generated(aiohttp_client, application, user_id):
    client = await aiohttp_client(application)
    response: client.ClientResponse = await client.post('/game', json={
        'user_id': user_id
    })
...
async def test_start_game_matches_two_players(aiohttp_client,application,user_id):
    client = await aiohttp_client(application)
